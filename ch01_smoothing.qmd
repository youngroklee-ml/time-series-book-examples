...
First, call `classical_decomposition()` inside `model()` function. The `model()` function returns `mable` object, which stands for "model table".

```{r}
electric_consumption_decomposition <- 
  electric_consumption_train |> 
  model(classical_decomposition(consumption))
...
electric_consumption_test |> 
  mutate(
    forecast_trend = predict(fit_poly, newdata = electric_consumption_test),
    forecast_seasonality = seasonality[quarter(year_quarter)],
