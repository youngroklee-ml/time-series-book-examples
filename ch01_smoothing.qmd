...
Visualize time series data. `autoplot()` for time series data frame draws line plot, where each line is corresponding to each `key` value.

```{r}
autoplot(forest_fire_ma_long)
```
